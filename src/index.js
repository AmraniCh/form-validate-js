(function() {
  var regex = {
    ...
    defaultMessages = {
      en: {
        ...
        extension: 'File extension must be {0}'
      },
      fr: {
  ...
  var FormValidator = function(form, settings) {
    ...
    if (settings) {
      typeof settings.constraints === 'object' && this.buildConstraints(settings.constraints);
    ...
    buildConstraints: function(constraints) {
      ...
      for (var filedName in constraints) {
        ...
        for (var constraintType in constraints[filedName]) {
          ...
          console.warn(
            constraintType + ' is unsupported validation constraint type.'
          );
      ...
      for (var key in ref) {
        ...
        for (var constraintType in constraint) {
          ...
          if (
            ...
            Object.keys(regex).indexOf(matchName) === -1
          ) {
            defaultMsg =
              (this.cutsomMatches &&
          ...
          if (typeof constraintMsg === 'undefined') {
            ...
            constraint.messages[constraintType] = constraintMsg.call(
              this,
              defaultMsg
            );
          }
          ...
          if (tokenValue && tokenRegex.test(constraintMsg)) {
            tokenValue =
              constraintType === 'equal' ? tokenValue.substr(1) : tokenValue;
            constraint.messages[constraintType] = constraintMsg.replace(
              tokenRegex,
              tokenValue
            );
          }
      ...
      if (events.indexOf('submit') !== -1) {
        this.bindEvent(this.form, 'submit', function(ev) {
          ...
          if (this.all() === true) {
            ...
            this.submitHandler
              ? this.submitHandler.apply(this, args)
              : this.form.submit();
          } else {
            // if fails
    ...
    handlers: {
      ...
      required: function(element, constraints) {
        var constraints = constraints.required; 
          isRequired = typeof constraints=== 'function' ? constraints.required.call(this, element) : constraints;
      ...
      extension: function(element, constraints) {
          if (element.type !== 'file') {
            console.warn(element.name + ' input is not of type file, thus the \'extension\' constraint type does nothing.');
          }
